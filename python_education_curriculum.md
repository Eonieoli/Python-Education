...
## 📅 과정 구성
- **기초 과정** (3일, 21시간): FastAPI 사용을 위한 Python 문법
...
- **클래스 활용** (FastAPI Pydantic 모델과 유사!)
  ...
  - 👉 Practice: 여러 인스턴스 생성
    ...
    ```
  - 👉 Practice: 인스턴스 리스트 관리
  - ✏️ Exercise: 클래스 활용 (2-3문제)

- **클래스 상속** (15분)
  - 상속이란? 부모 클래스의 기능 물려받기
  - 👉 Practice: 상속 기본 구조
    ```python
    # 부모 클래스
    class Animal:
  ...
  - **강사 설명:**
    "상속은 class 자식클래스(부모클래스): 형태로 씁니다.
    super().__init__()로 부모의 생성자를 호출할 수 있어요.
    
    응용 과정에서 Pydantic의 BaseModel을 상속받을 때
    '아! Day 3에서 본 그 모양이네!' 하실 거예요.
    
    지금은 이 '구조'만 기억하세요!"
  
  - ✏️ Exercise: 클래스 종합 (2-3문제)
...
- **Python 클래스 복습 + 딕셔너리 변환** (30분)
  ...
  - 👉 Practice: 딕셔너리 언패킹으로 클래스 생성
    ...
    # 방법 1: 하나씩 전달
    user = User(user_dict["name"], user_dict["age"], user_dict["email"])
    
    # 방법 2: 언패킹 (**) 사용
    user = User(**user_dict)  # ← ** 기호가 딕셔너리를 풀어줌!
    ```
  
  - **클래스 → 딕셔너리 변환** (수동)
  - 👉 Practice: 클래스를 딕셔너리로 변환
    ```python
    # 일반 Python: 수동으로 딕셔너리 만들어야 함
    user_dict = {
        "name": user.name,
        "age": user.age,
        "email": user.email
    ...
    # 또는 __dict__ 사용 (하지만 복잡할 수 있음)
    user_dict = user.__dict__
    ```
  
  - **강사 설명:**
    "Day 3에서 클래스 배웠죠?
    API 개발할 때는 JSON(딕셔너리)과 클래스를 자주 변환해야 해요.
    
    일반 Python은 이렇게 수동으로 해야 하는데,
    Pydantic을 쓰면 이게 전부 자동이에요!
    
    왜 Pydantic을 쓰는지 이제 이해되시죠?"
...
- **BaseModel 기본** (30분)
  - 👉 Practice: 첫 Pydantic 모델
    ...
    class User(BaseModel):  # ← Day 3에서 본 상속!
        name: str
        age: int
        email: str
    ```
  ...
  - **딕셔너리 → Pydantic (자동!)**
  - 👉 Practice: Pydantic으로 자동 변환
    ```python
    user_dict = {"name": "김철수", "age": 25, "email": "kim@example.com"}
...
- **선택적 필드** (20분)
  ...
  - 👉 Practice: 선택적 필드가 있는 모델
    ...
    class User(BaseModel):
        name: str
        age: int
        email: Optional[str] = None  # 선택적
        phone: str = "010-0000-0000"  # 기본값
    ```
...
- **Request Body**
  ...
  - 👉 Practice: Pydantic 자동 변환 확인
    ```python
    # 클라이언트가 JSON 보냄 → Pydantic이 자동 변환!
    # {"name": "김철수", "age": 25, "email": "kim@example.com"}
    # → User 객체로 변환됨
    ```
